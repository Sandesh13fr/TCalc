...
describe("recommendModels", () => {
  ...
  it("rejects invalid explicit output token counts while allowing zero", () => {
    for (const outputTokens of [-1, 1.5, Number.NaN, Number.POSITIVE_INFINITY]) {
      expect(() => recommendModels({ models: [cheapModel], workspaceTokens: 1000, goal: "debug", outputTokens })).toThrow(RangeError);
    }
