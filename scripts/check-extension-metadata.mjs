...
const copyAssetsPath = resolve(extRoot, "scripts/copy-assets.mjs");
...
const bundledRequired = [
  { rel: "README.md", src: "README.md" },
  { rel: "CHANGELOG.md", src: "CHANGELOG.md" },
  { rel: "LICENSE", src: "LICENSE" },
];

const needsBootstrap = bundledRequired.some(({ rel }) => {
  if (existsSync(resolve(extRoot, rel))) return false;
  if (!existsSync(resolve(repoRoot, rel))) return false;
  return true;
});

if (needsBootstrap) {
  if (!existsSync(copyAssetsPath)) {
    fail(
      `apps/vscode-extension/{README,CHANGELOG,LICENSE}.md are missing and copy-assets.mjs was not found at ${copyAssetsPath}.`,
    );
  } else {
    console.log("Bundled assets missing; running apps/vscode-extension/scripts/copy-assets.mjs ...");
    console.log("");
    const result = spawnSync(process.execPath, [copyAssetsPath], {
      stdio: "inherit",
      cwd: repoRoot,
    });
    if (result.status !== 0) {
      fail(`copy-assets.mjs exited with code ${result.status}`);
      console.log("");
...
const requiredFiles = bundledRequired.map((f) => ({ rel: f.rel, label: f.rel.replace(/\.md$/, "").replace(/^LICENSE$/, "LICENSE") }));
